<?php
	 ...
	 * units Class
	 *
	 * @used in  				units Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	units
	 */

	/*
	|--------------------------------------------------------------------------
	| units Function in Settings Module
	|--------------------------------------------------------------------------
	|
	...
	*/
	...
	class cUnits{
		...
		function units(){
			...
			break;
			case 'get_units':
				$returned_value = $this->_get_units();
			break;
			}
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'cunits.php';
		...
		private function _get_units(){
			...
			$all_units = execute_sql_query($query_settings);
			
			$units = array();
			
			if( is_array( $all_units ) && ! empty( $all_units ) ){
				
				foreach( $all_units as $category ){
					$units[ 'all' ][ $category[ 'id' ] ] = $category['name'];
					$units[ $category[ 'id' ] ] = $category;
				}
