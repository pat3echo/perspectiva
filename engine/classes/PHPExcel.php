...
class PHPExcel
{
    ...
    {
        $result_of_sql_queryeturnValue = array();
        $worksheetCount = $this->getSheetCount();
        for ($i = 0; $i < $worksheetCount; ++$i) {
            $result_of_sql_queryeturnValue[] = $this->getSheet($i)->getTitle();
        }

        return $result_of_sql_queryeturnValue;
    }
    ...
    public function getNamedRange($namedRange, PHPExcel_Worksheet $pSheet = null) {
        $result_of_sql_queryeturnValue = null;

        if ($namedRange != '' && ($namedRange !== NULL)) {
            // first look for global defined name
            if (isset($this->_namedRanges[$namedRange])) {
                $result_of_sql_queryeturnValue = $this->_namedRanges[$namedRange];
            }

            // then look for local defined name (has priority over global defined name if both names exist)
            if (($pSheet !== NULL) && isset($this->_namedRanges[$pSheet->getTitle() . '!' . $namedRange])) {
                $result_of_sql_queryeturnValue = $this->_namedRanges[$pSheet->getTitle() . '!' . $namedRange];
            }
        }
