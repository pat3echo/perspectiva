<?php
	 ...
	 * Website_menu Class
	 *
	 * @used in  				website_menu Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	website_menu
	 */

	/*
	|--------------------------------------------------------------------------
	| website_menu Function in Settings Module
	|--------------------------------------------------------------------------
	|
	...
	*/
	...
	class cWebsite_menu{
		...
		private $table_name = 'website_menu';
		...
		private $table_fields = array(
			'menu_position' => 'website_menu001',
			'menu_title_type' => 'website_menu002',
			'title_text' => 'website_menu003',
			'image_file' => 'website_menu004',
			'serial_num' => 'website_menu005',
			'url' => 'website_menu006',
			'tooltip' => 'website_menu007',
			'menu_type' => 'website_menu008',
		);
		...
		function website_menu(){
			...
			break;
			case 'get_website_menu':
				$returned_value = $this->_get_website_menu();
			break;
			}
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'cWebsite_menu.php';
		...
		private function _get_website_menu(){
			...
			$query = "SELECT ".$select." FROM `" . $this->class_settings['database_name'] . "`.`".$this->table_name."` where `record_status`='1' order by `".$this->table_fields['serial_num']."` asc ";
			$query_settings = array(
				'database' => $this->class_settings['database_name'] ,
				'connect' => $this->class_settings['database_connection'] ,
				'query' => $query,
			...
			$all_website_menu = execute_sql_query($query_settings);
			
			$website_menu = array();
			
			if( is_array( $all_website_menu ) && ! empty( $all_website_menu ) ){
				
				foreach( $all_website_menu as $category ){
					$website_menu[ $category[ 'menu_position' ] ][ $category['id'] ] = $category;
				}
				
				//Cache Settings
				$settings = array(
					'cache_key' => $cache_key,
					'cache_values' => $website_menu,
					'permanent' => true,
				);
