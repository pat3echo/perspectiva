<?php
	/**
	 * branch_offices Class
	 *
	 * @used in  				branch_offices Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	branch_offices
	 */

	/*
	|--------------------------------------------------------------------------
	| branch_offices Function in Settings Module
	|--------------------------------------------------------------------------
	|
	...
	*/
	...
	class cBranch_offices{
		public $class_settings = array();
		
		private $current_record_id = '';
		
		private $table_name = 'branch_offices';
		...
		private $table_fields = array(
			'name' => 'branch_offices001',
			'street_address' => 'branch_offices002',
			'city' => 'branch_offices003',
			'state' => 'branch_offices004',
			'country' => 'branch_offices005',
		);
		...
		function branch_offices(){
			...
			case 'get_branch_offices':
				$returned_value = $this->_get_branch_offices();
			break;
			}
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'cbranch_offices.php';
		...
		private function _get_branch_offices(){
			...
			$all_branch_offices = execute_sql_query($query_settings);
			
			$branch_offices = array();
			
			if( is_array( $all_branch_offices ) && ! empty( $all_branch_offices ) ){
				
				foreach( $all_branch_offices as $category ){
					$branch_offices[ 'all' ][ $category[ 'id' ] ] = $category['name'] . ' - ' . $category['city'];
					$branch_offices[ $category[ 'id' ] ] = $category;
				}
				
				//Cache Settings
				$settings = array(
					'cache_key' => $cache_key,
					'cache_values' => $branch_offices,
					'permanent' => true,
				);
