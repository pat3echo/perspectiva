<?php
	/**
	 * entry_exit_log Class
	 *
	 * @used in  				entry_exit_log Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	entry_exit_log
	 */

	/*
	|--------------------------------------------------------------------------
	| entry_exit_log Function in Settings Module
	|--------------------------------------------------------------------------
	|
	...
	*/
	...
	class cEntry_exit_log{
		public $class_settings = array();
		
		private $current_record_id = '';
		
		private $table_name = 'entry_exit_log';
		...
		private $table_fields = array(
			'visit_schedule_id' => 'entry_exit_log001',
			'entry_or_exit' => 'entry_exit_log002',
			'time' => 'entry_exit_log003',
			'photograph' => 'entry_exit_log004',
			'previous_time' => 'entry_exit_log005',
		);
		...
		function entry_exit_log(){
			...
			break;
			case 'get_entry_exit_log':
				$returned_value = $this->_get_entry_exit_log();
			break;
		...
		private function _get_entry_exit_log(){
			...
			if( isset( $this->class_settings['current_record_id'] ) && $this->class_settings['current_record_id'] ){
				...
				if( isset( $all_entry_exit_log[0] ) && is_array( $all_entry_exit_log[0] ) && ! empty( $all_entry_exit_log[0] ) ){
					
					//Cache Settings
					$settings = array(
						'cache_key' => $cache_key.'-'.$this->class_settings['current_record_id'],
						'directory_name' => $cache_key,
						'cache_values' => $all_entry_exit_log[0],
						'permanent' => true,
					);
					...
					//Cache Settings
					$settings = array(
						'cache_key' => $cache_key.'-currentstate-'.$all_entry_exit_log[0]['visit_schedule_id'],
						'directory_name' => $cache_key,
        ...
        private function _log_visitor_entry_or_exit(){
            ...
            if( isset( $this->class_settings['visit_schedule_id'] ) && $this->class_settings['visit_schedule_id'] ){
                ...
                $last_state = get_entry_exit_log_last_state( array( 'id' => $this->class_settings['visit_schedule_id'] ) );
                if( isset( $last_state ) && $last_state ){
                    $last_log_details = get_entry_exit_log_details( array( 'id' => $last_state ) );
                ...
                if( $save ){
					...
					return $this->_get_entry_exit_log();
                }
            }
        ...
        private function _get_recent_activties(){
            ...
            if( is_array( $all_entry_exit_log ) && ! empty( $all_entry_exit_log ) ){
                foreach( $all_entry_exit_log as $val ){
                    $details = get_entry_exit_log_details( array( 'id' => $val['id'] ) );
                    if( isset( $details['visit_schedule_id'] ) && $details['visit_schedule_id'] ){
                        
                        $visit = get_visit_schedule_details( array( 'id' => $details['visit_schedule_id'] ) );
                        if( is_array( $visit ) ){
                            $data[ doubleval( $val['creation_date'] ) ] = $visit;
                            
                            $data[ doubleval( $val['creation_date'] ) ]['date_time'] = date( "M d, Y h:ia" , doubleval( $details['time'] ) );
                            $data[ doubleval( $val['creation_date'] ) ]['entry'] = ( $details[ 'entry_or_exit' ] == 'entry' )?'in':'out';
            ...
            $employee_entry_exit_log->class_settings[ 'action_to_perform' ] = 'get_recent_activties';
            $d = $employee_entry_exit_log->employee_entry_exit_log();
                    ...
                    foreach( $all_entry_exit_log as $val ){
                        ...
                        if( isset( $details['visit_schedule_id'] ) && $details['visit_schedule_id'] ){
                            ...
                            if( is_array( $visit ) ){
                                ...
                                $data[ $val['id'] ]['whom_to_see'] = $visit['whom_to_see'];
                                $data[ $val['id'] ]['reason_for_visit'] = $visit['reason_for_visit'];
