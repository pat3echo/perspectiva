...
class Font_Metrics {
  ...
  static function save_font_families() {
    ...
    $cache_dt = var_export(self::$_font_lookup, true);
    $cache_dt = str_replace('\''.DOMPDF_FONT_DIR , 'DOMPDF_FONT_DIR . \'' , $cache_dt);
    $cache_dt = "<"."?php return $cache_dt ?".">";
    file_put_contents(self::CACHE_FILE, $cache_dt);
  }
  ...
  static function load_font_families() {
    ...
    if ( self::$_font_lookup === 1 ) {
      $cache_dt = file_get_contents(self::CACHE_FILE);
      file_put_contents(self::CACHE_FILE, "<"."?php return $cache_dt ?".">");
      self::$_font_lookup = require_once(self::CACHE_FILE);
    }
  }
